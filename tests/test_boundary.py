...
from exma.boundary.condition import minimum_image, periodic
...
class TestApply(unittest.TestCase):
    def test_pbc(self):
        ...
        """
        reference = np.array(
            [0.5, 0.5, 0.0, 1.0, 1.0, 0.0, 0.2, 0.8, 0.5], dtype=np.float32
        )

        cpc = periodic(np.array([1.0, 1.0, 1.0], dtype=np.float32))

        result = cpc.pbc(
            3,
            np.array(
                [-0.5, 0.5, 0.0, 1.0, 3.0, -1.0, 0.2, 0.8, 0.5],
                dtype=np.float32,
            ),
        )
